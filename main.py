...
class LoadingWindow(QWidget):
    def __init__(self, icon_path: Path):
        ...
        self.timer_label = BodyLabel("0.000")
        self.timer_label.setAlignment(Qt.AlignCenter)
        self.timer_label.setObjectName("loadingTimer")
        ...
        self.setStyleSheet("""
            ...
            #loadingSubtitle, #loadingHint, #loadingTimer { color: rgba(180, 180, 180, 235); }
        """)

    def _update_timer(self):
        elapsed = self._elapsed.elapsed()
        minutes, remainder = divmod(elapsed, 60000)
        seconds, milliseconds = divmod(remainder, 1000)
        if minutes:
            self.timer_label.setText(f"{minutes}.{seconds:02d}.{milliseconds:03d}")
        else:
            self.timer_label.setText(f"{seconds}.{milliseconds:03d}")
